...
# print('================= STATE 1: train models ===================')
 ...
 for cv in range(CrossValSplit):
  ...
  model=MakeModel(outDim,inDim,68,19)
#  LR=1e-3
...
#  if True:
  ...
  model.save('minimal_model_'+str(cv))
 quit()
 del model
...
# svd_model.summary()
     ...
     model=MakeModel(Ytr.shape[1],inp.shape[1],outp_hidden.shape[1],ifsvd=True,svd_model=svd_model,LayerName=LayerName,iftrain=False)
